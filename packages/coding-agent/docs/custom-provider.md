...
`reasoningReplayField` names the field reasoning is replayed under on assistant messages. Without it pi echoes reasoning back under the field the response streamed it in. Set `reasoningReplayField: "reasoning_content"` on vLLM: it streams `reasoning` now, but chat templates read `reasoning_content`, and a dropped replay surfaces no error.
